...
class Puzzle:
    def __init__(self, args):
        self.size = 0  # To store count of letter in a row (1 row)
    ...
    def parse_puzzle(self, puzzle_name):
        ...
        with open(puzzle_name, 'r') as pfile:
            ...
            for p_row in p_reader:
                puzzle.append(p_row)
            self.size = len(p_row[0]) - 1  # Update the self.size 
        return puzzle
    ...
    def find_diagonal(self, word):
        for a in range(0, len(self.puzzle[0][0])):
            ...
            i = 0
            j = self.size  # Just to keep the coords line clean & short
            while ((a - i) >= 0) and (i < len(self.puzzle)):
                # Replaced 29 with 'j' to work not just on 30x30 puzzle but on every size of puzzle 
